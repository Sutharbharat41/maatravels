...
const ReportsModule = () => {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `MAA_Travels_Ledger_${new Date().toISOString().slice(0, 10)}.xlsx`,
      );
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `MAA_Travels_Ledger_${new Date().toISOString().slice(0, 10)}.pdf`,
      );
      document.body.appendChild(link);
      link.click();
      link.parentNode.removeChild(link);
  ...
  const stats = payments.reduce(
    (acc, p) => {
      const basic = Number(p.basic_amount || 0);
      const commission = Number(p.commission_amount || 0);
      const tds = Number(p.tds_amount || 0);
      const otherDeduction = Number(p.deductions || 0);
      const net = Number(p.net_to_pay || 0);

      acc.gross += basic - commission;
      acc.tds += tds;
       acc.deductions += otherDeduction;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-500">
          Apply filtering criteria to review transactions and compile Excel /
          PDF business statements.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-slate-100 dark:bg-slate-900 p-5 rounded-3xl border border-slate-200/50 dark:border-slate-800/50">
        <span className="text-xs font-semibold text-slate-500">
          Showing {payments.length} ledger sheets matching filters. Download
          exports:
        </span>
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-initial px-5 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-md flex items-center justify-center space-x-1.5 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            disabled={downloading || payments.length === 0}
            className="flex-1 sm:flex-initial px-5 py-3 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold shadow-md flex items-center justify-center space-x-1.5 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <FileText size={16} />
      ...
      {/* Summary Box */}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-6 bg-slate-900 text-white p-6 rounded-3xl shadow-lg border border-slate-800">
  ...
  <div>
    ...
    <span className="text-lg sm:text-xl font-extrabold">
      Rs. {stats.gross.toLocaleString('en-IN')}
    </span>
  </div>
  ...
  <div>
    ...
    <span className="text-lg sm:text-xl font-extrabold text-rose-400">
      Rs. {stats.tds.toLocaleString('en-IN')}
    </span>
  </div>

  <div>
    <span className="block text-[10px] text-slate-400 font-bold uppercase">
      Commission + Other Deduction
    </span>
    <span className="text-lg sm:text-xl font-extrabold text-orange-400">
      Rs. {stats.deductions.toLocaleString('en-IN')}
    </span>
  </div>
  ...
  <div>
    ...
    <span className="text-lg sm:text-xl font-extrabold text-emerald-400">
      Rs. {stats.paid.toLocaleString('en-IN')}
    </span>
  </div>
  ...
  <div>
    ...
    <span className="text-lg sm:text-xl font-extrabold text-amber-400">
      Rs. {stats.pending.toLocaleString('en-IN')}
    </span>
  </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                {payments.map((p) => (
                  <tr
                    key={p.id}
                    className="text-slate-700 dark:text-slate-300 hover:bg-slate-50/50 dark:hover:bg-slate-900/50"
                  >
                    <td className="p-4 font-mono text-xs font-bold">
                      {p.vehicle_number}
                    </td>

                    <td className="p-4">{p.vehicle_name}</td>
                    ...
                    <td className="p-4 text-xs">
                      Rs. {Number(p.basic_amount || 0).toLocaleString("en-IN")}
                    </td>

                    <td className="p-4 font-extrabold text-slate-800 dark:text-white">
                      Rs. {Number(p.net_to_pay || 0).toLocaleString("en-IN")}
                    </td>
